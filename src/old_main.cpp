    ...
    void stimulate() {
        ...
        for (int arriveTime = 0; arriveTime <= maxTime; arriveTime ++) {
            ...
            while(!availableUsers.empty()) {
                ...
                bool f4 = availableUsers.size() >= 1;
                bool f5 = userRemainCount[userId] <= blockSize;
                bool f6 = freeBatchSize < maxBatchSize;
                if ((f1 and f2 and f3)) {
                    // 基本条件为有小块余量，并且不超时
...
void solve() {
    ...
    std::clog << "completed users: " << std::endl;
    for (int &v: ti) {
        std::clog << v << " ";
    }
    std::clog << std::endl;
    ...
    std::set<int> finalTimeoutUsers;
    for (int i = 1; i <= M; i ++) {
        finalTimeoutUsers.insert(i);
    }

    for (int i = 1; i <= N; i ++) {
        for (int j = 1; j <(int)npus[i].size(); j ++) {
            // std::vector<int> &ti = simulators[i][j].timeoutUsers;
            // timeoutUsers.insert(timeoutUsers.begin(), ti.begin(), ti.end());
            for (auto&k : simulators[i][j].completedUsers) {
                finalTimeoutUsers.erase(k);
                ans[k].insert(ans[k].end(), simulators[i][j].ans[k].begin(), simulators[i][j].ans[k].end());
            }
        }
    }

    std::clog << "after iter, timeout count: " << finalTimeoutUsers.size() << std::endl;
    std::clog << "timeout count: " << finalTimeoutUsers.size() << std::endl;
    
    for (auto &i: finalTimeoutUsers) {
